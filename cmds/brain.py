...
class Brain(Cog_Extension):
    ...
    @commands.command()
    async def password(self, ctx, number=4):
        member = ctx.author
        guessed = False
        times = number * 3

        if int(num.content) < 1 or int(num.content) > 9:
        ...
        while not guessed and times > 0:            
            ...
            if A == number:
                guessed = True
                await ctx.send(f"{member.mention} 你猜中了! 你猜的數字是 {''.join(data)}")
                return
